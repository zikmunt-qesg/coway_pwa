...
<div v-else class="sticky-top side-nav bg-white font-noto" style="z-index:5; top:3.5rem;">
    ...
    <div v-for="item in sub_titles" :key="item.id" class="mb-3 f-90 fw-400 line-height-low">
        <nuxt-link :to="toLink(item.hash)" :class="[active_color!='' ? new_hover:'', 'side-nav-link-3-en letter-narrow']" :style="isActive(safeHash(item.hash)) ? new_class : '' ">{{item.title}}</nuxt-link>
        <div v-for="child_item in item.child" :key="child_item.id" class="mt-2 f-85 fw-400 line-height-low">
            <nuxt-link :to="toLink(child_item.hash)" :class="[active_color!='' ? new_hover:'', 'side-nav-link-3-en d-flex']" :style="isActive(safeHash(child_item.hash)) ? new_class : '' " > 
                <i class="fas fa-angle-right pr-2 mt-1"></i><span class="letter-narrow" style="max-width:97%;">{{child_item.title}}</span>
            </nuxt-link>
        </div>
    </div>
</div>
