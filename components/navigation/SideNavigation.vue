...
<div v-if="!is_ENG" class="sticky-top side-nav bg-white font-noto" style="z-index:5; top:5.5rem;">
    ...
    <div v-for="item in sub_titles" :key="item.id" class="mb-3 f-90 fw-400 line-height-low">
        <nuxt-link :to="toLink(item.hash)" :class="[active_color!='' ? new_hover:'', 'side-nav-link-3 text-underline-effect letter-narrow']" :style="isActive(safeHash(item.hash)) ? new_class : '' ">{{item.title}}</nuxt-link>
