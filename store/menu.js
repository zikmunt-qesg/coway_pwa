
export const state = () => ({
    menu: [
        {
            title: 'MATERIAL TOPICS',
            link: '/static_page/Product-and-Service-Innovation',
            text: '사람들이 언제 어디서나 깨끗한 물과 공기로 안심하고 건강한 삶을 살 수 있도록 하는 일, 코웨이가 걸어 온 한결 같은 길입니다.',
            text_en: 'To help people live a healthy life with peace of mind and clean water anytime, anywhere, is the consistent path that Coway has walked.',
            child: [
                {
                    title: '제품과 서비스의 혁신',
                    title_en: 'Product and Service Innovation',
                    link: '/static_page/Product-and-Service-Innovation',
                    child:
                        [
                            {
                                title: '본질적 가치를 높이는 제품 혁신',
                                title_en: 'Product Innovation',
                                link: '/static_page/Product-and-Service-Innovation#a1'
                            },
                            {
                                title: '미래 기술 기반의 서비스 혁신',
                                title_en: 'Service Innovation',
                                link: '/static_page/Product-and-Service-Innovation#a2'
                            },
                            {
                                title: '혁신을 위한 연구개발 역량 강화',
                                title_en: 'Research Capacity Improvement',
                                link: '/static_page/Product-and-Service-Innovation#a3'
                            },

                        ]
                },
                {
                    title: '제품 책임',
                    title_en: 'Product Responsibility',
                    link: '/static_page/Product-Responsibility',
                    child:
                        [
                            {
                                title: '제품 책임 거버넌스',
                                title_en: 'Product Responsibility Governance',
                                link: '/static_page/Product-Responsibility#b1'
                            },
                            {
                                title: '제품 신뢰성 강화',
                                title_en: 'Product Reliability',
                                link: '/static_page/Product-Responsibility#b2'
                            },
                            {
                                title: '안전 인증 확대 및 고객 안심',
                                title_en: 'Safety Certification and Customer Satisfaction',
                                link: '/static_page/Product-Responsibility#b3'
                            }
                        ]
                },
                {
                    title: '정보보안 및 개인정보보호',
                    title_en: 'Information Security and Privacy Protection',
                    link: '/static_page/Information-Security-and-Personal-Information-Protection',
                    child:
                        [
                            {
                                title: '정보보안 및 개인정보보호 원칙과 조직',
                                title_en: 'Principles and Organizations',
                                link: '/static_page/Information-Security-and-Personal-Information-Protection#c1'
                            },
                            {
                                title: '정보보호 관리체계',
                                title_en: 'Information Security Mgt. System',
                                link: '/static_page/Information-Security-and-Personal-Information-Protection#c2'
                            },
                            {
                                title: '모니터링 및 실사 프로세스',
                                title_en: 'Monitoring and Due Diligence',
                                link: '/static_page/Information-Security-and-Personal-Information-Protection#c3'
                            }
                        ]
                }
            ]
        },
        {
            title: 'CARE FOR SUSTAINABILITY',
            link: '/static_page/CEO-Message',
            text: '비즈니스 전 과정에서 영향을 주고 받는 모든 이해관계자와 함께 새로운 가치를 더하는 일, 코웨이 지속가능경영의 방향입니다.',
            text_en: `To add new values with all stakeholders of the entire business process is the direction of Coway's Sustainability Management.`,
            child: [
                {
                    title: 'CEO Message',
                    title_en: 'CEO Message',
                    link: '/static_page/CEO-Message'
                },
                {
                    title: '가치창출 스토리',
                    title_en: 'Value Creation Story',
                    link: '/static_page/Value-Creation-Story',
                    child:
                        [
                            {
                                title: '비즈니스 모델',
                                title_en: 'Business Model',
                                link: '/static_page/Value-Creation-Story#d1'
                            },
                            {
                                title: '비즈니스 밸류체인',
                                title_en: 'Business Value Chain',
                                link: '/static_page/Value-Creation-Story#d2'
                            }
                        ]
                },
                {
                    title: '이해관계자 참여 및 중요 주제',
                    title_en: 'Stakeholder Engagement and Material Topics',
                    link: '/static_page/Stakeholder-Engagement-and-Material-Issues',
                    child:
                        [
                            {
                                title: '이해관계자 참여',
                                title_en: 'Stakeholder Engagement',
                                link: '/static_page/Stakeholder-Engagement-and-Material-Issues#e1'
                            },
                            {
                                title: '중대성 평가',
                                title_en: 'Materiality Assessment',
                                link: '/static_page/Stakeholder-Engagement-and-Material-Issues#e2'
                            },
                            {
                                title: '중대성 맥락과 중장기 목표',
                                title_en: 'Context and Long-Term Target',
                                link: '/static_page/Stakeholder-Engagement-and-Material-Issues#e3'
                            }
                        ]
                },
                {
                    title: '지배구조',
                    title_en: 'Corporate Governance',
                    link: '/static_page/Corporate-Governance',
                    child:
                        [
                            {
                                title: '이사회 중심의 책임경영 수행',
                                title_en: 'Board of Directors',
                                link: '/static_page/Corporate-Governance#f1'
                            },
                            {
                                title: '성과평가와 보수 지급',
                                title_en: 'Performance and Renumeration',
                                link: '/static_page/Corporate-Governance#f2'
                            },
                            {
                                title: '소유구조 및 의결권',
                                title_en: 'Ownership and Voting Rights',
                                link: '/static_page/Corporate-Governance#f3'
                            }
                        ]
                },
                {
                    title: '윤리경영',
                    title_en: 'Ethical Management',
                    link: '/static_page/Ethical-Management',
                    child:
                        [
                            {
                                title: '윤리강령',
                                title_en: 'Code of Conduct',
                                link: '/static_page/Ethical-Management#g1'
                            },
                            {
                                title: '윤리진단',
                                title_en: 'Compliance Check Organization',
                                link: '/static_page/Ethical-Management#g2'
                            },
                            {
                                title: '윤리교육',
                                title_en: 'Consensus by Ethics Education',
                                link: '/static_page/Ethical-Management#g3'
                            }
                        ]
                },
                {
                    title: '리스크 관리',
                    title_en: 'Risk Management',
                    link: '/static_page/Risk-Management',
                    child:
                        [
                            {
                                title: '전사적 리스크 관리 체계',
                                title_en: 'Company-wide Risk Mgt. System',
                                link: '/static_page/Risk-Management#h1'
                            },
                            {
                                title: '잠재 리스크의 관리',
                                title_en: 'Emerging Risks',
                                link: '/static_page/Risk-Management#h2'
                            }
                        ]
                },
                {
                    title: '경제적 가치 창출과 분배',
                    title_en: 'Economic Value Creation and Distribution',
                    link: '/static_page/Economic-Value-Creation-and-Distribution',
                    child:
                        [
                            {
                                title: '경제적 가치 창출',
                                title_en: 'Economic Value Creation',
                                link: '/static_page/Economic-Value-Creation-and-Distribution#j1'
                            },
                            {
                                title: '경제적 가치 분배',
                                title_en: 'Economic Value Distribution',
                                link: '/static_page/Economic-Value-Creation-and-Distribution#j2'
                            },
                            {
                                title: '조세 정책',
                                title_en: 'Tax Policy',
                                link: '/static_page/Economic-Value-Creation-and-Distribution#j3'
                            }
                        ]
                },
            ]
        },
        {
            title: 'CARE FOR ENVIRONMENT',
            link: '/static_page/Environmental-Management-for-Clean-Water-and-Air',
            text: '비즈니스 운영 과정에서 환경영향을 최소화하고 환경을 건강하게 지키는 일, 그린 글로벌 리더 코웨이가 함께합니다.',
            text_en: 'To minimize environmental impact and keep the environment healthy in the course of business operations, is the responsibility of Green Global Leader Coway.',
            child: [
                {
                    title: '깨끗한 물과 공기를 위한 환경경영',
                    title_en: 'Environmental Management',
                    link: '/static_page/Environmental-Management-for-Clean-Water-and-Air',
                    child:
                        [
                            {
                                title: '환경경영을 통한 깨끗함의 가치 실현',
                                title_en: 'Environmental Mgt. for Cleanliness',
                                link: '/static_page/Environmental-Management-for-Clean-Water-and-Air#k1'
                            },
                            {
                                title: '가치사슬 전체의 환경역량 강화',
                                title_en: 'Value Chain Environmental Competence',
                                link: '/static_page/Environmental-Management-for-Clean-Water-and-Air#k2'
                            }
                        ]
                },
                {
                    title: '친환경 제품 개발과 제품 전 과정 관리',
                    title_en: 'Eco-friendly Product Development and Life Cycle Mgt.',
                    link: '/static_page/Eco-Friendly-Product-Development-and-Product-Life-Cycle-Management',
                    child:
                        [
                            {
                                title: '친환경 제품과 서비스, 비즈니스 설계',
                                title_en: 'Eco-friendly Design',
                                link: '/static_page/Eco-Friendly-Product-Development-and-Product-Life-Cycle-Management#m1'
                            },
                            {
                                title: '자원효율성 향상과 순환경제 구축',
                                title_en: 'Resource Efficiency and Circular Economy',
                                link: '/static_page/Eco-Friendly-Product-Development-and-Product-Life-Cycle-Management#m2'
                            }
                        ]
                },
                {
                    title: '기후변화 대응',
                    title_en: 'Climate Change',
                    link: '/static_page/Responses-to-Climate-Change',
                    child:
                        [
                            {
                                title: '기후변화 목표 및 시나리오 관리',
                                title_en: 'Climate Change Goals',
                                link: '/static_page/Responses-to-Climate-Change#n1'
                            },
                            {
                                title: '기후변화의 위험과 기회 분석',
                                title_en: 'Risks and Opportunities of Climate Change',
                                link: '/static_page/Responses-to-Climate-Change#n2'
                            },
                            {
                                title: '협력사 탄소 파트너십',
                                title_en: 'Carbon Partnership with Supplier',
                                link: '/static_page/Responses-to-Climate-Change#n3'
                            },
                            {
                                title: '온실가스 · 에너지 및 기타 환경 데이터',
                                title_en: 'GHG, Energy and Other Environment Data',
                                link: '/static_page/Responses-to-Climate-Change#n4'
                            }
                        ]
                }
            ]
        },
        {
            title: 'CARE FOR SOCIETY',
            link: '/static_page/Employee-Centered-Company-Culture-and-Safe-Working-Environment',
            text: '우리가 하는 일이 더 나은 세상, 더 좋은 사회, 더 행복한 삶을 만드는 데 기여하는지 코웨이는 확인하고 노력하겠습니다.',
            text_en: 'Coway will make sure that our work contributes to a better world, a better society, and happier life.',
            child:
                [
                    {
                        title: '임직원 중심의 기업문화와 안전한 근무환경',
                        title_en: 'Employee-oriented Culture, Safe and Healthy Workplace',
                        link: '/static_page/Employee-Centered-Company-Culture-and-Safe-Working-Environment',
                        child:
                            [
                                {
                                    title: '착한성장을 위한 역량 강화',
                                    title_en: 'Systematic Enhancement',
                                    link: '/static_page/Employee-Centered-Company-Culture-and-Safe-Working-Environment#r1'
                                },
                                {
                                    title: '커뮤니케이션 기반의 기업문화',
                                    title_en: 'Communication-based Culture',
                                    link: '/static_page/Employee-Centered-Company-Culture-and-Safe-Working-Environment#r2'
                                },
                                {
                                    title: '안전하고 건강한 근무환경',
                                    title_en: 'Safe and Healthy Workplace',
                                    link: '/static_page/Employee-Centered-Company-Culture-and-Safe-Working-Environment#r3'
                                }
                            ]
                    },
                    {
                        title: '라이프케어 전문 파트너 양성',
                        title_en: 'Lifecare Professional Partners',
                        link: '/static_page/Train-Lifecare-Professional-Partners',
                        child:
                            [
                                {
                                    title: '파트너 전문성 강화',
                                    title_en: 'Partner Expertise Enhancement',
                                    link: '/static_page/Train-Lifecare-Professional-Partners#s1'
                                },
                                {
                                    title: '파트너 상생 및 커뮤니케이션',
                                    title_en: 'Cooperation and Communication',
                                    link: '/static_page/Train-Lifecare-Professional-Partners#s2'
                                }
                            ]
                    },
                    {
                        title: '동반성장',
                        title_en: 'Mutual Growth',
                        link: '/static_page/Win-Win',
                        child:
                            [
                                {
                                    title: '협력사와의 소통을 통한 성장 지원',
                                    title_en: 'Communication with Suppliers',
                                    link: '/static_page/Win-Win#t1'
                                },
                                {
                                    title: '공정거래 문화 조성',
                                    title_en: 'Promoting Fair Trade Practice',
                                    link: '/static_page/Win-Win#t2'
                                }
                            ]
                    },
                    {
                        title: '지역사회',
                        title_en: 'Local Community',
                        link: '/static_page/Local-Community',
                        child:
                            [
                                {
                                    title: '코웨이 다운 사회공헌',
                                    title_en: 'Contribution based on Goodness',
                                    link: '/static_page/Local-Community#u1'
                                },
                                {
                                    title: '깨끗한 물과 공기를 위한 코웨이의 약속',
                                    title_en: 'Commitment for Clean Water and Air',
                                    link: '/static_page/Local-Community#u2'
                                },
                                {
                                    title: '공공정책 참여',
                                    title_en: 'Policy Influence',
                                    link: '/static_page/Local-Community#u3'
                                }
                            ]
                    },
                    {
                        title: '협력사 CSR 리포트',
                        title_en: 'Supplier CSR Report',
                        link: '/static_page/Supplier-CSR-Report',
                        child:
                            [
                                {
                                    title: '협력사 행동 규범',
                                    title_en: 'Supplier Code of Conduct',
                                    link: '/static_page/Supplier-CSR-Report#v1'
                                },
                                {
                                    title: '협력사 지속가능성 평가',
                                    title_en: 'Supplier Sustainability Assessment',
                                    link: '/static_page/Supplier-CSR-Report#v2'
                                },
                                {
                                    title: '분쟁광물',
                                    title_en: 'Conflict Minerals',
                                    link: '/static_page/Supplier-CSR-Report#v3'
                                }
                            ]
                    },
                    {
                        title: '인권 리포트',
                        title_en: 'Human Rights Report',
                        link: '/static_page/Human-Rights-Report',
                        child:
                            [
                                {
                                    title: '인권경영 원칙 및 가이드',
                                    title_en: 'Human Rights Principles',
                                    link: '/static_page/Human-Rights-Report#w1'
                                },
                                {
                                    title: '인권 리스크 식별 및 완화 프로세스',
                                    title_en: 'Risk Identification and Mitigation',
                                    link: '/static_page/Human-Rights-Report#w2'
                                }
                            ]
                    }
                ]
        },
        {
            title: 'DATA AND FRAMEWORKS',
            link: '/static_page/Policies-and-Initiatives',
            child:
                [
                    {
                        title: '정책 및 이니셔티브',
                        title_en: 'Policies and Initiatives',
                        link: '/static_page/Policies-and-Initiatives',
                        child:[
                            {
                                title: '지속가능성 정책',
                                title_en: 'Sustainability Policy',
                                link: '/static_page/Policies-and-Initiatives'
                            },
                            {
                                title: '참여 이니셔티브',
                                title_en: 'Initiatives',
                                link: '/static_page/Policies-and-Initiatives?p=Initiatives'
                            },
                        ]
                    },
                    {
                        title: '보고 프레임워크 인덱스',
                        title_en: 'Reporting Frameworks',
                        link: '/framework/gri',
                        child:[
                            {
                                title: 'GRI',
                                title_en: 'GRI',
                                link: '/framework/gri'
                            },
                            {
                                title: 'SASB',
                                title_en: 'SASB',
                                link: '/framework/sasb'
                            },
                            {
                                title: 'DJSI Public',
                                title_en: 'DJSI Public',
                                link: '/framework/djsi'
                            }
                        ]
                    },
                    {
                        title: '보고 범위 및 검증',
                        title_en: 'About This Report and Assurance',
                        link: '/static_page/About-This-Report-and-Assurance',
                        child:[
                            {
                                title: 'About This Report',
                                title_en: 'About This Report',
                                link: '/static_page/About-This-Report-and-Assurance'
                            },
                            {
                                title: '제3자 검증의견서',
                                title_en: 'Independent Assurance Statement',
                                link: '/static_page/About-This-Report-and-Assurance?p=Independent-Assurance-Statement'
                            },
                            {
                                title: '온실가스 검증의견서',
                                title_en: 'GHG Assurance Statement',
                                link: '/static_page/About-This-Report-and-Assurance?p=GHG-Assurance-Statement'
                            }
                        ]
                    },
                    {
                        title: '코웨이 지속가능경영 뉴스',
                        title_en: 'Sustainability News',
                        link: '/news'
                    },
                    {
                        title: '보고서 다운로드',
                        title_en: 'Downloads',
                        link: '/downloads'
                    }
                ]
        }

    ]
})